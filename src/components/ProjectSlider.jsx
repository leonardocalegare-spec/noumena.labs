...
import { getCarouselDirection, wrapCarouselIndex } from '../lib/carousel.js'
...
export default function ProjectSlider({ slides = [], title }) {
  ...
  const show = (index, movement) => {
    const next = wrapCarouselIndex(index, slides.length)
    if (next === active) return
    setDirection(getCarouselDirection(active, next, slides.length, movement))
    setActive(next)
  }

  const move = (amount) => show(active + amount, amount)
  ...
  return (
    <div
      ...
      aria-label={`Galeria visual do projeto ${title}. Arraste para trocar as evidências.`}
    >
      <div className={`project-slide slide-${direction}`} key={current.image}>
        <img
          src={current.image}
          alt={`${title}: ${current.description}`}
          width={current.width}
      ...
      <div className="slider-controls">
        <button
          ...
          type="button"
          onClick={() => move(-1)}
          aria-label="Evidência visual anterior"
        >
        ...
        <button className="slider-arrow" type="button" onClick={() => move(1)} aria-label="Próxima evidência visual">
          <Icon name="arrow" size={18} />
        </button>
      </div>
    </div>
